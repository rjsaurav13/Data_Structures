...
#include<conio.h>
#define SIZE 5
int push(int arr[],int *temp,int ele);  // function prototype for insertion
...
void main()
{
 ...
 break;
 case 2:del=pop(s,&top); // call pop function to delete top most element of the stack
 if(del!=-1)
...
int push(int arr[],int *temp,int ele)
{
...
{
 int i;
 if(SIZE==-1) // check if stack is empty
 {
